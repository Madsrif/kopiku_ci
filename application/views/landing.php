...
<html lang="en">
  ...
  <nav class="navbar navbar-expand-lg bg-white sticky-top">
      <div class="container position-relative py-2">
        <a class="navbar-brand position-absolute" href="#"><img src="<?php echo base_url('resource/img/logo2.png');?>" height="55px"></a>
        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNavAltMarkup" aria-controls="navbarNavAltMarkup" aria-expanded="false" aria-label="Toggle navigation">
          <span class="navbar-toggler-icon"></span>
        </button>
        <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
            <div class="navbar-nav mx-auto" id="menu-data"></div>
        </div>
...
<header class="container pt-5">
    <h1 id="hero-tagline" class="text-center">Madsrif Coffee <br> a Cup of Coffee</h1>
...
<section id="our-coffee">
    ...
    <section id="location">
      <div class="container py-5">
        <div class="row border p-5" style="--bs-border-width: 7px; --bs-gutter-x: 2rem;">
          <div class="col-6 d-flex justify-content-center">
            <img width="100%" src="<?php echo base_url('resource/img/location.png');?>" alt="">
          </div>
          <div class="col-6 col-6 d-flex justify-content-center align-items-center">
            <div>
    ...
    <section id="gallery">
      <div class="row" style="--bs-gutter-x: 0rem;">
        <div class="col-4">
          <img width="100%" src="<?php echo base_url('resource/img/gallery 1.png'); ?>">
        </div>
        <div class="col-4">
          <img width="100%" src="<?php echo base_url('resource/img/gallery 2.png'); ?>">
        </div>
        <div class="col-4">
          <img width="100%" src="<?php echo base_url('resource/img/gallery 3.png'); ?>">
        </div>
      </div>
    </section>
    ...
    <section id="footer-image" style="background-color:#F3F4F6; background-position: bottom center; background-repeat: no-repeat">
      <div class="text-center" style="padding-top: 7rem; padding-bottom: 7rem;">
        <h1 id="footer-tagline">Coffee with a friend is like capturing happiness in a cup.</h1>
      </div>
    </section>
  ...
  <script>
    ...
    $.get("http://localhost:1337/api/coffes?populate=*&pagination[limit]=3", function(data, status){
     console.log(data) 
     let dataHtml='';
     data.data.map((e)=>{
          dataHtml += `
          <div class="col-4">
            <div class="img-card">
              <img width="100%" src="http://localhost:1337${e.attributes.picture.data.attributes.url}">
              <div class="hover">
            ...
            <div class="py-3 text-center"> ${e.attributes.description} </div>
          </div>`
        })
    ...
    // <!-- Instagram -->
    $.get("http://localhost:1337/api/instagrams?populate=*&pagination[limit]=5", function(data, status){
     console.log(data) 
     let dataHtml='';
     data.data.map((e)=>{
          dataHtml += `
    ...
    $.get("http://localhost:1337/api/about?populate=*", function(data, status){
    
    $("#hero-tagline").html(data.data.attributes.tagline)
    $("#hero-img").attr('src', 'http://localhost:1337' +data.data.attributes.banner.data.attributes.url)

    $("#about-description").html(data.data.attributes.description)
    $("#about-button").html(data.data.attributes.button_text)
    $("#about-img").attr('src', 'http://localhost:1337'+data.data.attributes.picture.data.attributes.url)

})

$.get("http://localhost:1337/api/setting?populate=*", function(data, status){
...
data.data.attributes.page_menu.map((e)=>{
   dataMenu += `<a class="nav-link">${e.title}</a>`
})
